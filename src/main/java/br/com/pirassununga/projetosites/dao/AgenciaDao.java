...
public class AgenciaDao implements AgenciaIf {
    ...
    public boolean adicionar(Agencia agencia) {
        ...
        try {
            conex = new ConexaoBD();
            pst = conex.getConnection().prepareStatement("INSERT into agencia(nome,numero_ag,telefone,uf,cidade,rua,numero)"
                    + "VALUES (?,?,?,?,?,?,?)");
            pst.setString(1, agencia.getNome());
            pst.setString(2, agencia.getNumero());
            pst.setString(3, agencia.getTelefone());
            pst.setString(4, agencia.getEndereco().getUf());
            pst.setString(5, agencia.getEndereco().getCidade());
            pst.setString(6, agencia.getEndereco().getRua());
            pst.setInt(7, agencia.getEndereco().getNumero());
